...
def get_latest_registration_resume_task(
    db: Session,
    email: str,
    *,
    email_service_id: Optional[int] = None,
    ...
    if email_service_id is not None:
        query = query.filter(RegistrationTask.email_service_id == email_service_id)

    tasks = query.order_by(desc(RegistrationTask.created_at)).limit(max(1, int(limit or 100))).all()
    for task in tasks:
        if exclude_task_uuid and str(task.task_uuid or "").strip() == str(exclude_task_uuid).strip():
            continue

        result = task.result if isinstance(task.result, dict) else {}
        task_email = str(result.get("email") or "").strip().lower()
        if task_email != email_norm:
            continue

        metadata = result.get("metadata") if isinstance(result.get("metadata"), dict) else {}
        resume_state = str(metadata.get("resume_state") or "").strip().lower()

        if task.status == "completed":
            return None
        if resume_state in {"invalidated", "resume_invalidated", "consumed", "resume_consumed"}:
            return None
        if resume_state in {"pending_resume", "suspected_resume"} and str(result.get("password") or "").strip():
            return task
