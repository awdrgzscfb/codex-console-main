...
class AccountSessionTokenSaveRequest(BaseModel):
    session_token: str
...
@router.get("/{account_id}/session-diagnostic")
async def get_account_session_diagnostic(
    account_id: int,
    ...
    return payment_routes.build_account_session_diagnostic_payload(account_id, probe=probe, proxy=proxy)


@router.post("/{account_id}/session-bootstrap")
async def bootstrap_account_session_token(
    account_id: int,
    proxy: Optional[str] = Query(None, description="会话补全代理"),
):
    from . import payment as payment_routes

    return payment_routes.build_account_session_bootstrap_payload(account_id, proxy=proxy)


@router.post("/{account_id}/relogin-repair")
async def relogin_account_session_token(
    account_id: int,
    proxy: Optional[str] = Query(None, description="自动重登修复代理"),
):
    from . import payment as payment_routes

    return payment_routes.build_account_relogin_repair_payload(account_id, proxy=proxy)


@router.post("/{account_id}/session-token")
async def save_account_session_token(
    account_id: int,
    request: AccountSessionTokenSaveRequest,
):
    from . import payment as payment_routes

    save_request = payment_routes.SaveSessionTokenRequest(
        session_token=request.session_token,
        merge_cookie=bool(request.merge_cookie),
    )
    return payment_routes.save_account_session_token_payload(account_id, save_request)
