...
def _normalize_email_identity(email: Optional[str]) -> str:
    return str(email or "").strip().lower()


def _find_account_by_email(db, email: Optional[str]) -> Optional[AccountModel]:
    normalized_email = _normalize_email_identity(email)
...
def _resolve_effective_email_service_id(db, task_uuid: str, email_service_id: Optional[int]) -> Optional[int]:
    if email_service_id:
        return int(email_service_id)

    task = crud.get_registration_task(db, task_uuid)
    if task and task.email_service_id:
        try:
...
def _run_sync_registration_task(task_uuid: str, email_service_type: str, proxy: Optional[str], email_service_config: Optional[dict], email_service_id: Optional[int] = None, log_prefix: str = "", batch_id: str = "", auto_upload_cpa: bool = False, cpa_service_ids: List[int] = None, auto_upload_sub2api: bool = False, sub2api_service_ids: List[int] = None, auto_upload_tm: bool = False, tm_service_ids: List[int] = None, auto_upload_new_api: bool = False, new_api_service_ids: List[int] = None, registration_type: str = RoleTag.CHILD.value):
    ...
    with get_db() as db:
        ...
        try:
            ...
            effective_email_service_id = _resolve_effective_email_service_id(db, task_uuid, email_service_id)

            # 优先使用数据库中配置的邮箱服务
            if effective_email_service_id:
                from ...database.models import EmailService as EmailServiceModel
                db_service = db.query(EmailServiceModel).filter(
                    EmailServiceModel.id == effective_email_service_id,
...
def _init_batch_state(batch_id: str, task_uuids: List[str]):
    ...
    batch_tasks[batch_id] = {
        ...
        "success": 0,
        "failed": 0,
        "skipped": int(existing.get("skipped", 0) or 0),
        "cancelled": bool(existing.get("cancelled", False)),
        "service_ids": list(existing.get("service_ids", []) or []),
