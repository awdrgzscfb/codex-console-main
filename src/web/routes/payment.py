...
def _bootstrap_session_token_by_relogin(db, account: Account, proxy: Optional[str]) -> str:
    ...
    try:
        ...
        login_otp_ok = engine._verify_email_otp_with_retry(
            stage_label="会话补全验证码",
            max_attempts=1,
            fetch_timeout=120,
        ...
        if not login_otp_ok:
            engine._log("会话补全验证码仍未命中，尝试重触发登录 OTP 后再校验...", "warning")
            if not engine._retrigger_login_otp():
                logger.warning(
                    "会话补全登录重触发 OTP 失败，停止继续收码: account_id=%s email=%s",
                    account.id,
                    account.email,
                )
                return ""
            login_otp_ok = engine._verify_email_otp_with_retry(
                stage_label="会话补全验证码(重发)",
                max_attempts=3,
                fetch_timeout=120,
                attempted_codes=login_otp_tried_codes,
            )
...
def build_account_session_diagnostic_payload(
    account_id: int,
    probe: bool = True,
    proxy: Optional[str] = None,
):
...
def build_account_session_bootstrap_payload(
    account_id: int,
    proxy: Optional[str] = None,
):
    with get_db() as db:
...
def build_account_relogin_repair_payload(
    account_id: int,
    proxy: Optional[str] = None,
):
    with get_db() as db:
        ...
        runtime_proxy = _resolve_runtime_proxy(proxy, account)
        token = _bootstrap_session_token_by_relogin(db, account, runtime_proxy)
        if not token:
            return {
                "success": False,
                "message": "自动重登修复未命中 session_token",
                "account_id": account.id,
                "email": account.email,
            }
...
def save_account_session_token_payload(
    account_id: int,
...
def get_account_session_diagnostic(
    ...
    """
    return build_account_session_diagnostic_payload(account_id, probe=probe, proxy=proxy)
...
def bootstrap_account_session_token(
    ...
    """
    return build_account_session_bootstrap_payload(account_id, proxy=proxy)


@router.post("/accounts/{account_id}/relogin-repair")
def relogin_account_session_token(
    account_id: int,
    proxy: Optional[str] = Query(None, description="自动重登修复代理"),
):
    """
    主动触发一次自动重登修复：
    直接走邮箱 + 密码 + OTP 登录链路补齐会话。
    """
    return build_account_relogin_repair_payload(account_id, proxy=proxy)
...
def save_account_session_token(
    ...
    """
    return save_account_session_token_payload(account_id, request)
