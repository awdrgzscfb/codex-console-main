...
class RegistrationEngine:
    ...
    def __init__(
        ...
        self._password_registered_successfully: bool = False
        self._username_rejected_but_login_password: bool = False
        self._resume_candidate_invalidated: bool = False
        self._resume_candidate_invalidated_reason: str = ""
        self._resume_source_task_uuid: str = ""
        self._resume_source_state: str = ""
    ...
    def _get_bound_email_service_id(self) -> Optional[int]:
        if not self.task_uuid:
            return None
        try:
            with get_db() as db:
                task = crud.get_registration_task_by_uuid(db, self.task_uuid)
                if not task or getattr(task, "email_service_id", None) is None:
                    return None
                return int(task.email_service_id)
        except Exception:
            return None

    def _load_pending_resume_candidate(self) -> Optional[Dict[str, Any]]:
        email_text = str(self.email or "").strip()
        if not email_text:
            return None

        try:
            with get_db() as db:
                task = crud.get_latest_registration_resume_task(
                    db,
                    email_text,
                    email_service_id=self._get_bound_email_service_id(),
                    exclude_task_uuid=self.task_uuid,
                )
                if not task:
                    return None
                ...
                return {
                    ...
                    "password": password,
                    "state": str(metadata.get("resume_state") or "").strip(),
                    "reason": str(metadata.get("resume_reason") or "").strip(),
                }
        except Exception as e:
            self._log(f"加载待续跑记录失败: {e}", "warning")
            return None

    def _mark_resume_candidate_invalidated(self, reason: str) -> None:
        self._resume_candidate_invalidated = True
        self._resume_candidate_invalidated_reason = str(reason or "").strip()

    def _attempt_pending_resume_login(
        self,
        did: str,
        ...
        self._resume_source_task_uuid = str(candidate.get("task_uuid") or "").strip()
        self._resume_source_state = str(candidate.get("state") or "").strip()
        self.password = str(candidate.get("password") or "").strip()
        if not self.password:
            return False

        self._log(
            f"命中待续跑记录，优先尝试登录续跑（来源任务: {self._resume_source_task_uuid[:8] or 'unknown'}）",
            "warning",
        )
        ...
        if not login_start_result.success:
            self._log(
                f"待续跑登录入口失败，先不判定密码失效，回退新注册: {login_start_result.error_message}",
                "warning",
            )
            return False
        ...
        if page_type == OPENAI_PAGE_TYPES["EMAIL_OTP_VERIFICATION"]:
            ...
            self._log("待续跑记录已直达邮箱验证码页，转入登录续跑", "warning")
            return True

        if page_type != OPENAI_PAGE_TYPES["LOGIN_PASSWORD"]:
            self._mark_resume_candidate_invalidated(f"登录入口返回非预期页面: {page_type or 'unknown'}")
            self._log(
                f"待续跑登录未进入密码页/验证码页（{page_type or 'unknown'}），本次判定该待续跑记录失效",
                "warning",
            )
            return False
        ...
        if password_result.success and password_result.is_existing_account:
            self._is_existing_account = True
            self._resume_used = True
            self._log("待续跑密码验证通过，已进入邮箱验证码页", "warning")
            return True

        error_text = str(password_result.error_message or "").strip()
        self._mark_resume_candidate_invalidated(
            error_text or f"密码续跑未进入验证码页: {password_result.page_type or 'unknown'}"
        )
        self._log(
            f"待续跑密码未通过，本次判定该待续跑记录失效，回退新注册: {error_text or password_result.page_type or 'unknown'}",
            "warning",
        )
        return False

    def _attach_resume_metadata(self, result: RegistrationResult) -> RegistrationResult:
        result.email = str(result.email or self.email or "")
        result.password = str(result.password or self.password or "")
        result.device_id = str(result.device_id or self.device_id or "")

        metadata = dict(result.metadata or {})
        if self._resume_used:
            metadata["resumed_from_task_uuid"] = self._resume_source_task_uuid
            metadata["resumed_from_state"] = self._resume_source_state

        if not result.success:
            if self._resume_candidate_invalidated:
                metadata["resume_state"] = "invalidated"
                metadata["resume_reason"] = (
                    self._resume_candidate_invalidated_reason or "saved_resume_candidate_no_longer_works"
                )
                metadata["resume_source_task_uuid"] = self._resume_source_task_uuid
                metadata["resume_source_state"] = self._resume_source_state
            elif self._password_registered_successfully and result.password:
                metadata["resume_state"] = "pending_resume"
                metadata["resume_reason"] = "password_submitted_but_flow_not_completed"
            elif self._username_rejected_but_login_password and result.password:
                metadata["resume_state"] = "suspected_resume"
                metadata["resume_reason"] = "username_rejected_but_login_password_probe"
    ...
    def _register_password(self, did: Optional[str] = None, sen_token: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            self._username_rejected_but_login_password = False
            # 生成密码
                        ...
                        if did:
                            ...
                            try:
                                ...
                                if probe.success and probe.page_type == OPENAI_PAGE_TYPES["EMAIL_OTP_VERIFICATION"]:
                                    self._log("登录入口探测命中邮箱 OTP：该邮箱可确认为既有 OpenAI 账号", "warning")
                                    self._mark_email_as_registered()
                                    self._last_register_password_error = (
                                        "该邮箱已存在 OpenAI 账号。"
                                    ...
                                    self._username_rejected_but_login_password = True
                                    self._log(
                                        "登录入口仅命中密码页：这只能说明邮箱疑似存在，暂不标记为已注册",
                                        "warning",
                                    )
                                    self._last_register_password_error = (
                                        "登录入口仅返回密码页，无法确认该邮箱已完成注册；"
                                        "本轮不会将其标记为已注册。"
                                        "若这是中断注册，请优先使用上一轮任务日志里的“生成密码”走登录续跑；"
                                        "否则建议更换邮箱重试。"
                                    )
            ...
            self._password_registered_successfully = True
    ...
    def _run_primary_registration(self) -> RegistrationResult:
        ...
        try:
            ...
            self._last_validate_otp_continue_url = None
            self._last_validate_otp_workspace_id = None
            self._password_registered_successfully = False
            self._username_rejected_but_login_password = False
            self._resume_candidate_invalidated = False
            self._resume_candidate_invalidated_reason = ""
            self._resume_source_task_uuid = ""
            self._resume_source_state = ""
            ...
            resume_candidate = self._load_pending_resume_candidate()
            resume_login_ready = False
            if resume_candidate:
                resume_login_ready = self._attempt_pending_resume_login(did, sen_token, resume_candidate)

            # 4. 提交注册入口邮箱
            if not resume_login_ready:
            ...
            return self._attach_resume_metadata(result)
    ...
    def run(self) -> RegistrationResult:
        ...
        if primary_result.success:
            return self._attach_resume_metadata(primary_result)
        ...
        if fallback_result.success:
            ...
            return self._attach_resume_metadata(fallback_result)
