...
from ...config.constants import AccountStatus, OAUTH_CLIENT_ID
...
class TokenRefreshManager:
    ...
    @staticmethod
    def _is_rotated_refresh_token_error(message: Optional[str]) -> bool:
        text = str(message or "").strip().lower()
        if not text:
            return False
        return (
            "refresh token has already been used" in text
            or "please try signing in again" in text
            or "refresh_token has already been used" in text
        )
    ...
    def refresh_by_session_token(
        ...
        Args:
            session_token: 会话令牌
            refresh_token: 可选 OAuth Refresh Token（用于一次性会话令牌场景兜底）
            client_id: 可选 OAuth Client ID
        ...
        try:
            ...
            # 某些一次性/异常 session_token 链路会返回 grant 错误，若提供了 refresh_token 则兜底走 OAuth 刷新
            if response.status_code >= 400:
                ...
                if (
                    ...
                    and refresh_token
                    and error_text in {"invalid_grant", "unsupported_grant_type", "invalid_request"}
                ):
                    oauth_client_id = client_id or self.settings.openai_client_id or OAUTH_CLIENT_ID
    ...
    def refresh_by_oauth_token(
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 401 and self._is_rotated_refresh_token_error(error_message):
                    ...
                    result.error_message = f"OAuth token 刷新失败: HTTP {response.status_code}{code_text}{detail}"
    ...
    def refresh_account(self, account: Account) -> TokenRefreshResult:
        ...
        if account.refresh_token:
            ...
            if (
                (not result.success)
                and self._is_rotated_refresh_token_error(result.error_message)
                and account.access_token
            ):
                is_valid, validate_error = self.validate_token(account.access_token, timeout_seconds=15)
                if is_valid:
                    result.error_message = (
                        "OAuth Refresh Token 已失效：数据库里的 refresh_token 是旧值。"
                        "不过当前 access_token 仍然可用，账号暂时还能正常使用；"
                        "后续请重新登录或补会话，拿到最新 refresh_token。"
                    )
                    logger.warning(result.error_message)
                elif validate_error:
                    result.error_message = (
                        f"{result.error_message}；当前 access_token 也不可用: {validate_error}"
                    )
            return result
    ...
    def validate_token(self, access_token: str, timeout_seconds: int = 30) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return False, "订阅受限（402）"
            elif response.status_code == 403:
                # 403 在当前业务里通常代表工作区/权限受限，但账号可继续使用。
                return True, None
            else:
...
def validate_account_token(
    ...
    with get_db() as db:
        ...
        if not account.access_token:
            # 无 Token 直接标记为 failed，便于账号管理筛选定位。
