                ...
                <div style="display:flex;gap:4px;align-items:center;white-space:nowrap;">
                    ...
                    <div class="dropdown" style="position:relative;">
                        ...
                        <div class="dropdown-menu" style="min-width:100px;">
                            ...
                            <a href="#" class="dropdown-item" onclick="event.preventDefault();closeMoreMenu(this);bootstrapSessionToken(${account.id})">补会话</a>
                            <a href="#" class="dropdown-item" onclick="event.preventDefault();closeMoreMenu(this);reloginRepair(${account.id})">重登修复</a>
    ...
    try {
        ...
        elements.modalBody.innerHTML = `
            ...
            <div style="margin-top: var(--spacing-lg); display: flex; gap: var(--spacing-sm);">
                ...
                <button class="btn btn-secondary" onclick="bootstrapSessionToken(${id})">
                    🔐 补会话
                </button>
                <button class="btn btn-secondary" onclick="reloginRepair(${id})">
                    ♻️ 重登修复
                </button>
                <button class="btn btn-secondary" onclick="showSessionDiagnostic(${id})">
                    🩺 会话诊断
                </button>
            </div>
        `;
...
async function bootstrapSessionToken(id) {
    try {
        const result = await api.post(`/accounts/${id}/session-bootstrap`, {});
        if (result && result.success) {
            toast.success(result.message || 'Session Token 补全成功');
        } else {
...
async function reloginRepair(id) {
    try {
        const result = await api.post(`/accounts/${id}/relogin-repair`, {});
        if (result && result.success) {
            toast.success(result.message || '自动重登修复成功');
        } else {
            toast.warning(result?.message || '自动重登修复未成功');
        }
    } catch (error) {
        toast.error('自动重登修复失败: ' + error.message);
    } finally {
        await viewAccount(id);
        loadAccounts();
    }
...
async function showSessionDiagnostic(id) {
    try {
        const result = await api.get(`/accounts/${id}/session-diagnostic`);
        const diagnostic = result?.diagnostic || {};
        const notes = Array.isArray(diagnostic.notes) ? diagnostic.notes : [];
        const recommendation = String(diagnostic.recommendation || '无建议').trim();
        const tokenState = diagnostic.token_state || {};
        const lines = [
            `建议: ${recommendation}`,
            `Access Token: ${tokenState.has_access_token ? '有' : '无'}`,
            `Refresh Token: ${tokenState.has_refresh_token ? '有' : '无'}`,
            `Session Token(DB): ${tokenState.has_session_token_db ? '有' : '无'}`,
            `Session Token(Cookie): ${tokenState.has_session_token_cookie ? '有' : '无'}`,
        ];
        if (notes.length > 0) {
            lines.push('', '备注:');
        ...
        toast.error('会话诊断失败: ' + error.message);
    }
}
